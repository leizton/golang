...
runtime_gentraceback(byte *pc0, byte *sp, byte *lr0, G *g, int32 skip, uintptr *pcbuf, int32 max)
{
	...
	if(pc == (uintptr)runtime_goexit) {
		pc = (uintptr)g->entry;
		lr = (uintptr)runtime_goexit;
	}
	...
	for(iter = 0; iter < 100 && n < max; iter++) {	// iter avoids looping forever
		...
		if(pc <= 0x1000 || (f = runtime_findfunc(pc)) == nil) {
			...
			p = (byte*)pc;
			if((pc&3) == 0 && p < p+4 &&
			   runtime_mheap.arena_start < p &&
			   p+4 < runtime_mheap.arena_used) {
			   	x = *(uintptr*)p;
				...
				if((x&0xfffff000) == 0xe52de000 && lr == (uintptr)runtime_goexit) {
					...
					if((byte*)pc < p && p < p+4 && p+4 < runtime_mheap.arena_used) {
						pc = *(uintptr*)p;
						fp = nil;
						continue;
					}
		...
		if(skip > 0)
			...
			if(runtime_showframe(f)) {
				...
				for(i = 0; i < f->args; i++) {
					if(i != 0)
						runtime_prints(", ");
					runtime_printhex(((uintptr*)fp)[1+i]);
					if(i >= 4) {
						runtime_prints(", ...");
						break;
					}
		...
		if(pcbuf == nil && f->entry == (uintptr)runtime_newstack && g == m->g0) {
			runtime_printf("----- newstack called from goroutine %d -----\n", m->curg->goid);
			pc = (uintptr)m->morepc;
...
void
runtime_traceback(byte *pc0, byte *sp, byte *lr, G *g)
{
	runtime_gentraceback(pc0, sp, lr, g, 0, nil, 100);
}
...
runtime_callers(int32 skip, uintptr *pcbuf, int32 m)
{
	...
	sp = runtime_getcallersp(&skip);
	pc = runtime_getcallerpc(&skip);

	return runtime_gentraceback(pc, sp, 0, g, skip, pcbuf, m);
}
